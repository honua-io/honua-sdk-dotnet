...
# Release and verify its integrity. This is the SDK-side consumer of the shared
# fixture-distribution mechanism delivered in geospatial-grpc#19: the canonical
# conformance fixtures live in geospatial-grpc and are published as a versioned
# release asset; this repo pulls a pinned version rather than copying or forking
# the fixture files into the tree.
#
# It downloads the release asset
#   conformance-fixtures-<version>.tar.gz
# (and its .sha256), verifies the tarball checksum, extracts it, re-verifies
# every packaged file against the in-tarball SHA256SUMS, and asserts the
# embedded VERSION matches the requested pin. The result is a directory
# containing fixtures/ (+ manifest.txt), golden/, run.sh, and VERSION for that
# exact geospatial.v1 schema release.
#
# Usage:
#   conformance/fetch-fixtures.sh --version 0.2.0-alpha.1 [--dest DIR] [--repo OWNER/REPO]
#
...
# Geospatial.Grpc package version (see conformance/PINS.md).
#
# Equivalent raw pull (no helper):
#   base=https://github.com/honua-io/geospatial-grpc/releases/download/v<version>
#   curl -fsSLO $base/conformance-fixtures-<version>.tar.gz
#   curl -fsSLO $base/conformance-fixtures-<version>.tar.gz.sha256
#   sha256sum -c conformance-fixtures-<version>.tar.gz.sha256
#   tar -xzf conformance-fixtures-<version>.tar.gz
#
...
echo "  run.sh          buf-based schema verification harness (needs CONFORMANCE_IMAGE)"
