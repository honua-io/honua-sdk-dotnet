...
/// into this repo; CI fetches the selected pinned or canary release asset and
/// points the suite at the extracted directory via <c>HONUA_CONFORMANCE_FIXTURES_DIR</c>.
/// </summary>
public static class ConformanceFixtures
{
    private const string ExpectedVersionEnv = "HONUA_CONFORMANCE_EXPECTED_FIXTURE_VERSION";
        ...
        if (File.Exists(versionFile))
        {
            ...
            if (!string.Equals(version, expectedVersion, StringComparison.Ordinal))
            {
                throw new InvalidOperationException(
                    $"Fixture VERSION ({version}) does not match the expected conformance version ({expectedVersion}). " +
                    $"Keep {ExpectedVersionEnv}, conformance/FIXTURE_VERSION, and Directory.Build.props aligned for the selected lane.");
            }
        }
    ...
    public static string? GetSkipReason()
        => ResolveRoot() is null
            ? $"Set {FixturesDirEnv} to an extracted conformance-fixtures-{GetExpectedVersion()} bundle " +
              $"(conformance/fetch-fixtures.sh --version {GetExpectedVersion()}) to run schema conformance."
            : null;
    ...
    {
        var expectedVersion = Environment.GetEnvironmentVariable(ExpectedVersionEnv);
