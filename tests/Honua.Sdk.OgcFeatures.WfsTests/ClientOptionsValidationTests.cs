...
public sealed class ClientOptionsValidationTests
{
    ...
    public void ValidateBaseAddress_NullUri_Throws()
    {
        Assert.Throws<HonuaConfigurationException>(
            () => HonuaWfsClientOptions.ValidateBaseAddress(null));
    }
    ...
    {
        Assert.Throws<HonuaConfigurationException>(
            () => HonuaWfsClientOptions.ValidateBaseAddress(new Uri("/relative", UriKind.Relative)));
    }
    ...
    {
        Assert.Throws<HonuaConfigurationException>(
            () => HonuaWfsClientOptions.ValidateBaseAddress(new Uri(value)));
    }
    ...
    {
        Assert.Throws<HonuaConfigurationException>(
            () => HonuaWfsClientOptions.ValidateTimeout(TimeSpan.FromMilliseconds(millis)));
    }
    ...
    {
        Assert.Throws<HonuaConfigurationException>(
            () => HonuaWfsClientOptions.ValidateTimeout(TimeSpan.FromHours(24)));
    }

    [Fact]
    public void ValidateTimeout_AcceptsTypicalValue()
    {
        HonuaWfsClientOptions.ValidateTimeout(TimeSpan.FromSeconds(30));
    }
    ...
    {
        var options = new HonuaWfsClientOptions { MaxRetryAttempts = attempts };
    ...
    {
        Assert.True(HonuaWfsClientOptions.RequiresHttpsForAuthentication(null));
    }

    [Fact]
    public void RequiresHttpsForAuthentication_Https_ReturnsFalse()
    {
        Assert.False(HonuaWfsClientOptions.RequiresHttpsForAuthentication(new Uri("https://example.com")));
    }
    ...
    {
        Assert.False(HonuaWfsClientOptions.RequiresHttpsForAuthentication(new Uri(value)));
    }

    [Fact]
    public void RequiresHttpsForAuthentication_RemoteHttp_ReturnsTrue()
    {
        Assert.True(HonuaWfsClientOptions.RequiresHttpsForAuthentication(new Uri("http://example.com")));
    }

    [Fact]
    public void IsLocalDevelopmentHttp_HttpsScheme_ReturnsFalse()
    {
        Assert.False(HonuaWfsClientOptions.IsLocalDevelopmentHttp(new Uri("https://localhost:5000")));
    }

    [Fact]
    public void IsLocalDevelopmentHttp_LoopbackHttp_ReturnsTrue()
    {
        Assert.True(HonuaWfsClientOptions.IsLocalDevelopmentHttp(new Uri("http://127.0.0.1")));
    }
}
