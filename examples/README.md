...
| [OfflineConflictConsole](OfflineConflictConsole/) | Offline/sync integrators | Runnable deterministic sample | `OfflineSyncEngine` conflict envelope production, detection, and ServerWins/ClientWins/ManualReview merge-resolve paths with in-memory stores | `dotnet run --project examples/OfflineConflictConsole/OfflineConflictConsole.csproj` |
| [FieldFormConsole](FieldFormConsole/) | Field data-collection integrators | Runnable deterministic sample | `FormValidator` validation plus `CalculatedFieldEvaluator` calculated-field evaluation against a portable `FormDefinition`/`FieldRecord` | `dotnet run --project examples/FieldFormConsole/FieldFormConsole.csproj` |
