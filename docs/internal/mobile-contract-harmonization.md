...
|-----------------|---------------------|--------|
| `honua-mobile` source packages from `main` after `honua-mobile#69` | `Honua.Sdk.*` `0.1.7-alpha.1` | Fixture-level compatibility for shared feature, source, edit, attachment, routing, scene, field, and offline contracts |
...
- New portable field form code targets `Honua.Sdk.Field` for form definitions,
...
- `honua-mobile#56` replaces reusable mobile field contracts with the published
  `Honua.Sdk.Field` package while keeping MAUI/runtime capture behavior in
  mobile.
- `honua-sdk-dotnet#72` graduates plugin contracts into an SDK package after
  linked server dependencies are ready.
