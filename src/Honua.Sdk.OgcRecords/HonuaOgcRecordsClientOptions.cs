...
/// </summary>
public sealed class HonuaOgcRecordsClientOptions : IHonuaAuthenticationOptions
{
    /// <summary>
    ...
    internal static void ValidateBaseAddress(Uri? baseAddress)
    {
        ...
        {
            throw new InvalidOperationException("Honua OGC Records base address must be configured.");
        }

        if (!baseAddress.IsAbsoluteUri)
        {
            throw new InvalidOperationException("Honua OGC Records base address must be an absolute URI.");
        }
        ...
        {
            throw new InvalidOperationException("Honua OGC Records base address must use HTTP or HTTPS.");
        }
    }
    ...
    internal static void ValidateTimeout(TimeSpan timeout)
    {
        if (timeout <= TimeSpan.FromMilliseconds(10) || timeout >= TimeSpan.FromHours(24))
        {
            throw new InvalidOperationException(
                "Honua OGC Records timeout must be greater than 10 milliseconds and less than 24 hours.");
        }
    }
