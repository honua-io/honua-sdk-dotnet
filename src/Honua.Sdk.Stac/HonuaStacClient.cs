...
public sealed class HonuaStacClient : IHonuaStacClient
{
    ...
    /// <inheritdoc />
    public async Task<StacLandingPage> GetLandingPageAsync(CancellationToken ct = default)
    {
        var body = await GetStringAsync(BasePath, ct).ConfigureAwait(false);
    ...
    /// <inheritdoc />
    public Task<StacLandingPage> GetCatalogAsync(CancellationToken ct = default)
        => GetLandingPageAsync(ct);

    /// <inheritdoc />
    public async Task<IReadOnlyList<StacCollection>> ListCollectionsAsync(CancellationToken ct = default)
    {
        var body = await GetStringAsync($"{BasePath}/collections", ct).ConfigureAwait(false);
    ...
    /// <inheritdoc />
    public async Task<StacCollection> GetCollectionAsync(string collectionId, CancellationToken ct = default)
    {
    ...
    public async Task<StacItemCollection> SearchAsync(
        ...
        CancellationToken ct = default)
    {
        var body = await GetStringAsync($"{BasePath}/search{BuildQueryString(query)}", ct).ConfigureAwait(false);
    ...
    public async Task<StacItemCollection> SearchAsync(
        ...
        CancellationToken ct = default)
    {
        var body = await PostJsonStringAsync($"{BasePath}/search", request ?? new StacSearchRequest(), ct)
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        while (true)
        {
            ...
            var body = await GetNextPageStringAsync(nextLink, ct).ConfigureAwait(false);
    ...
    public async IAsyncEnumerable<StacItemCollection> SearchPagesAsync(
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        var page = await SearchAsync(query, ct).ConfigureAwait(false);
        yield return page;
    ...
    public async IAsyncEnumerable<StacItemCollection> SearchPagesAsync(
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        var page = await SearchAsync(request, ct).ConfigureAwait(false);
        yield return page;
    ...
    public async Task<HttpResponseMessage> SearchRawAsync(
        StacSearchQuery? query = null,
        CancellationToken ct = default)
        => await _http.GetAsync(CreateRequestUri($"{BasePath}/search{BuildQueryString(query)}"), ct)
            .ConfigureAwait(false);
    ...
    private async Task<string> PostJsonStringAsync(string url, StacSearchRequest request, CancellationToken ct)
    {
    ...
    private async Task<string> GetNextPageStringAsync(StacLink nextLink, CancellationToken ct)
    {
        ValidateNextLinkOrigin(nextLink.Href);
        return await GetStringAsync(nextLink.Href, ct).ConfigureAwait(false);
    }
