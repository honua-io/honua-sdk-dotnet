...
/// </summary>
public sealed class HonuaStacClientOptions : IHonuaAuthenticationOptions
{
    /// <summary>
    ...
    /// <summary>
    /// Default OAuth/OIDC scopes requested by STAC clients.
    /// </summary>
    public IReadOnlyList<string> AuthenticationScopes { get; set; } = Array.Empty<string>();

    /// <summary>
    /// Default OAuth/OIDC audience or resource requested by STAC clients.
    /// </summary>
    ...
    internal static void ValidateBaseAddress(Uri? baseAddress)
    {
        ...
        {
            throw new InvalidOperationException("Honua STAC base address must be configured.");
        }

        if (!baseAddress.IsAbsoluteUri)
        {
            throw new InvalidOperationException("Honua STAC base address must be an absolute URI.");
        }
        ...
        {
            throw new InvalidOperationException("Honua STAC base address must use HTTP or HTTPS.");
        }
    }
    ...
    internal static void ValidateTimeout(TimeSpan timeout)
    {
        if (timeout <= TimeSpan.FromMilliseconds(10) || timeout >= TimeSpan.FromHours(24))
        {
            throw new InvalidOperationException(
                "Honua STAC timeout must be greater than 10 milliseconds and less than 24 hours.");
        }
    }
