...
public sealed class HonuaSpecClientOptions : IHonuaAuthenticationOptions
{
    ...
    internal static void ValidateBaseAddress(Uri? baseAddress)
    {
        ...
        {
            throw new InvalidOperationException("Honua spec base address must be configured.");
        }

        if (!baseAddress.IsAbsoluteUri)
        {
            throw new InvalidOperationException("Honua spec base address must be an absolute URI.");
        }
        ...
        {
            throw new InvalidOperationException("Honua spec base address must use HTTP or HTTPS.");
        }
    }
    ...
    internal static void ValidateTimeout(TimeSpan timeout)
    {
        if (timeout <= TimeSpan.FromMilliseconds(10) || timeout >= TimeSpan.FromHours(24))
        {
            throw new InvalidOperationException(
                "Honua spec timeout must be greater than 10 milliseconds and less than 24 hours.");
        }
    }
